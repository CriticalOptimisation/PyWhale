...
if __name__ == "__main__":
    from pywhale import Api, General, Live, Turbo
else:
    from .pywhale import Api, General, Live, Turbo
...
class PyWhale(Api, General, Live, Turbo):
    ...
    def _testSymbols(self, symb):
        if symb != '' and len(symb.split(',')) > 5:
            print(
                'Error, You can only request information for up to 5 elements at once. Lower your input number and retry\n')
            return False
        else:
            return True
    ...
    def _updateKey(self, key):
        ...
        if key in l:
            i = l.index(key)
            k = [self.BTC_real_key, self.BTC_demo_key, self.DASH_real_key, self.DASH_demo_key, self.ETH_real_key,
                 self.LTC_real_key]
            key = k[i]
        else:
            print(
